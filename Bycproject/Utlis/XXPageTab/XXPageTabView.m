...
#define HEIGHTS(view) view.bounds.size.height
#define WIDTHS(view) view.bounds.size.width
...
- (void)layoutSubviews {
    if(_isNeedRefreshLayout) {
        ...
        //body layout
        self.bodyView.frame = CGRectMake(0, _tabSize.height, WIDTHS(self), HEIGHTS(self)-_tabSize.height);
        self.bodyView.contentOffset = CGPointMake(self.frame.size.width*_selectedTabIndex, 0);
        self.bodyView.contentSize = CGSizeMake(WIDTHS(self)*_numberOfTabItems, 0);
        ...
        for(NSInteger i = 0; i < _numberOfTabItems; i++) {
            ...
            childController.view.frame = CGRectMake(WIDTHS(self)*i, 0, WIDTHS(self), HEIGHTS(self)-_tabSize.height);
        }
    }
}
...
- (void)initTabView {
    ...
    for(NSInteger i = 0; i < _numberOfTabItems; i++) {
        ...
        if (i==self.selectedTabIndex) {
            tabItem.font = [UIFont systemFontOfSize:19];
        }
        else
        {
            tabItem.font = [UIFont systemFontOfSize:14];
        }
...
- (void)finishReviseTabContentOffset {
    ...
    _isChangeByClick = NO;
    XXPageTabItemLable *currentTabItem = _tabItems[_selectedTabIndex];
    currentTabItem.font = [UIFont systemFontOfSize:19];
...
- (void)changeSelectedItemToNextItem:(NSInteger)nextIndex {
    ...
    if (nextIndex==self.selectedTabIndex) {
        currentTabItem.font = [UIFont systemFontOfSize:19];
    }
    else
    {
        currentTabItem.font = [UIFont systemFontOfSize:14];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if(scrollView == self.bodyView) {
        _selectedTabIndex = self.bodyView.contentOffset.x/WIDTHS(self.bodyView);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(scrollView == self.tabView) {
        ...
        if(self.indicatorView.superview) {
            XXPageTabItemLable *selecedTabItem = _tabItems[_selectedTabIndex];
            self.indicatorView.frame = CGRectMake(selecedTabItem.center.x-WIDTHS(self.indicatorView)/2.0-scrollView.contentOffset.x, ORIGIN_Y(self.indicatorView), WIDTHS(self.indicatorView), HEIGHTS(self.indicatorView));
        }
        ...
        if(!_isChangeByClick) {
            ...
            } else if(self.bodyView.contentOffset.x >= self.bodyView.contentSize.width-WIDTHS(self.bodyView)) { //右边界
                _leftItemIndex = _numberOfTabItems-1;
                _rightItemIndex = _numberOfTabItems-1;
                
            } else {
                _leftItemIndex = (int)(self.bodyView.contentOffset.x/WIDTHS(self.bodyView));
...
- (void)changeTitleWithDefault {
    CGFloat relativeLocation = self.bodyView.contentOffset.x/WIDTHS(self.bodyView)-_leftItemIndex;
    if(!_isChangeByClick) {
...
- (void)changeTitleWithGradient {
    if(_leftItemIndex != _rightItemIndex) {
        CGFloat rightScale = (self.bodyView.contentOffset.x/WIDTHS(self.bodyView)-_leftItemIndex)/(_rightItemIndex-_leftItemIndex);
...
- (void)changeTitleWithBlend {
    CGFloat leftScale = self.bodyView.contentOffset.x/WIDTHS(self.bodyView)-_leftItemIndex;
    if(leftScale == 0) {
...
- (void)changeIndicatorFrame {
    ...
    CGFloat nowIndicatorCenterX = _tabItemWidth*(0.5+self.bodyView.contentOffset.x/WIDTHS(self.bodyView));
    //计算此时body的偏移量在一页中的占比
    CGFloat relativeLocation = (self.bodyView.contentOffset.x/WIDTHS(self.bodyView)-_leftItemIndex)/(_rightItemIndex-_leftItemIndex);
    //记录左右对应的indicator宽度
    ...
    self.indicatorView.frame = CGRectMake(nowIndicatorCenterX-nowIndicatorWidth/2.0-_tabView.contentOffset.x, ORIGIN_Y(self.indicatorView), nowIndicatorWidth, HEIGHTS(self.indicatorView));
}
...
- (void)changeIndicatorFrameByStretch {
    ...
    //当前的frame
    CGRect nowFrame = CGRectMake(0, ORIGIN_Y(self.indicatorView), 0, HEIGHTS(self.indicatorView));
...
- (void)setSelectedTabIndex:(NSInteger)selectedTabIndex {
    if(selectedTabIndex >= 0 && selectedTabIndex < _numberOfTabItems && _selectedTabIndex != selectedTabIndex) {
        ...
        [self resetMainView];
        [self initMainView];
        self.bodyView.contentOffset = CGPointMake(WIDTHS(self)*_selectedTabIndex, 0);
...
- (void)setSelectedColor:(UIColor *)selectedColor {
    ...
    self.indicatorView.backgroundColor = [UIColor colorWithHexString:@"FF0000"];
